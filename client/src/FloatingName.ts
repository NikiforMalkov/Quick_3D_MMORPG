...
export class FloatingName extends Component {
  ...
  visible_ = false;
  sprite_:any;
  element_ :HTMLCanvasElement|undefined;
  context2d_:any;
  ...
  InitComponent() {
    this._RegisterHandler('load.character', (m:any) => {
  ...
  CreateSprite_(msg:any) {
    if (!this.visible_) {
      return
    }
    const modelData = CHARACTER_MODELS[this.params_.desc.character.class as keyof typeof CHARACTER_MODELS]
