...
export class NetworkController extends Component {
  ...
  public playerID_:any;
  private socket_: io.Socket | null = null;
  ...
  SetupSocket_() {
    this.socket_ = io.io('ws://localhost:3000', {
      reconnection: false,
    ...
    this.socket_.on('connect', () => {
      ...
      this.socket_!.emit('login.commit', document.getElementById('login-input')!.getAttribute('value'))
    })
