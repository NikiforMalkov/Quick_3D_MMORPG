...
class _TerrainBuilderThreadedWorker {
  ...
  Init(params:any) {
    this._params = params
    this._params.offset = new Vector3(params.offset[0], params.offset[1], params.offset[2])
  ...
  Rebuild() {
    ...
    // Fix the skirt
    const _ApplyFix = (x:any, y:any, xp:any, yp:any) => {
    ...
    function _Unindex(src:any, stride:any) {
      const dst = []
      for (let i = 0, n = indices.length; i < n; i += 3) {
