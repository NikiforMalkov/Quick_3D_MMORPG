...
export class UIInventoryController extends Component {
  ...
  InitEntity() {
    ...
    const _SetupElement = (n:any) => {
      const element:any = document.getElementById(n)
      element.ondragstart = (ev:any) => {
        ev.dataTransfer.setData('text/plain', n)
      }
      element.ondragover = (ev:any) => {
        ev.preventDefault()
      }
      element.ondrop = (ev:any) => {
        ev.preventDefault()
  ...
  OnItemDropped_(oldElement:any, newElement:any) {
  ...
  SetItemAtSlot_(slot:any, itemName:string) {
