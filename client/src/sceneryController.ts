...
export class SceneryController extends Component {
  ...
  SpawnClouds_() {
    for (let i = 0; i < 20; ++i) {
      ...
      const pos = new Vector3((Math.random() * 2.0 - 1.0) * 5000, 500, (Math.random() * 2.0 - 1.0) * 5000)
      ...
      const q = new Quaternion().setFromAxisAngle(new Vector3(0, 1, 0), math.rand_range(0, 360))
      e.SetQuaternion(q)
  ...
  FindBiome_(terrain:any, pos:any) {
  ...
  SpawnAt_(biome:string, spawnPos:any) {
  ...
  SpawnCrap_() {
    ...
    const center = new Vector3().copy(player.Position)
