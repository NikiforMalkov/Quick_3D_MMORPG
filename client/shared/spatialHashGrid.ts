...
import {WorldEntityInterface} from "../../shared/worldEntityInterface.js";
...
export interface NodeInterface {
    next: null | SpatialHashGridClient,
    prev: null | SpatialHashGridClient,
...
export interface DismensionsInterface {
...
export class SpatialHashGrid {
    ...
    _GetCellIndex(position:number[]) {
        const x = sat((position[0] - this._bounds[0][0]) / (this._bounds[1][0] - this._bounds[0][0]))
        const y = sat((position[1] - this._bounds[0][1]) / (this._bounds[1][1] - this._bounds[0][1]))
    ...
    NewClient(position:number[], dimensions:number[]) : SpatialHashGridClient {
        const client:any = {
            position: position,
    ...
    FindNear(position:number[], bounds:number[]) {
        const [x, y] = position
        const [w, h] = bounds
        ...
        for (let x = i1[0], xn = i2[0]; x <= xn; ++x) {
            for (let y = i1[1], yn = i2[1]; y <= yn; ++y) {
                ...
                while (head) {
                    const v:SpatialHashGridClient = head.client
                    head = head.next
    ...
    _Insert(client:SpatialHashGridClient) {
        const [x, y] = client.position
        const [w, h] = client.dimensions
    ...
    Remove(client:SpatialHashGridClient) {
        if (client._cells.min == null || client._cells.max == null || client._cells.nodes == null) return;
